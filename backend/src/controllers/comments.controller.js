...
export async function getAllComments(req, res) {
    const comments = await commentService.getComments({ postId: req.params.postId, page: parseInt(req.query.page), size: parseInt(req.query.size) });
...
export async function newComment(req, res) {
    ...
    if (!comment.type === "comment") return res.status(400).json({ error: "Not a comment" });
    if (!validComment(comment)) return res.status(400).json({ error: 'Missing required property' });
